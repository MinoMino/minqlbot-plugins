...
class serverratings(minqlbot.Plugin):
    ...
    def print_ratings(self, names, channel, game_type):
        ...
        teams = self.teams()
        red_sorted = sorted(teams["red"], key=lambda x: balance.cache[x.name.lower()][game_type]["elo"], reverse=True)
        blue_sorted = sorted(teams["blue"], key=lambda x: balance.cache[x.name.lower()][game_type]["elo"], reverse=True)
        red = "^7" + ", ".join(["{}: ^1{}^7".format(p, balance.cache[p.name.lower()][game_type]["elo"]) for p in red_sorted])
        blue = "^7" + ", ".join(["{}: ^4{}^7".format(p, balance.cache[p.name.lower()][game_type]["elo"]) for p in blue_sorted])
